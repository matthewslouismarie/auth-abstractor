<?php

namespace Tests\LM;

use Firehed\U2F\Registration;
use InvalidArgumentException;
use LM\Authentifier\Factory\U2fRegistrationFactory;
use LM\Authentifier\Implementation\Member;
use LM\Common\DataStructure\TypedMap;
use LM\Common\Enum\Scalar;
use PHPUnit\Framework\TestCase;

class U2fRegistrationFactoryTest extends TestCase
{
    public function testFactory()
    {
        $regFactory = new U2fRegistrationFactory();
        $firehedReg = (new Registration())
            ->setCounter(0)
            ->setAttestationCertificate(base64_decode(''))
            ->setPublicKey(base64_decode('BAcdB+X8+hq8MulBDtyfknw+bJsjyrK74dGuVg2hx6gSjFg3rHrhcH6J92r6qCBRYogNo04eeSV5XwwquVGFpFI='))
            ->setKeyHandle(base64_decode('PeTDOgdeJiftM3YOMVzr4lBEdMoR+wRdYARe8eWnuSB9V8VeD1wjcRkhbOadiZBSh/J/7XrQN4h31PjOaK+JwA=='))
        ;
        $u2fReg = $regFactory->fromFirehed($firehedReg);
        $this->assertEquals(
            $firehedReg,
            $regFactory->toFirehed($u2fReg)
        );
    }
}
